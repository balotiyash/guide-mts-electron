...
import { generateInvoice, printInvoice, openInvoiceInBrowser } from '../services/invoiceService.js';
...
function registerInvoiceHandler() {
    ...
    // New handler for browser-based invoice printing
    ipcMain.handle('open-invoice-in-browser', async (event, userId, workId, type) => {
        try {
            console.log('open-invoice-in-browser called with:', { userId, workId, type }); // Debug log
            const customer = await paymentService.getPaymentsByUserId(userId, workId, type);
            if (!customer || !customer.payments || customer.payments.length === 0) {
                throw new Error('No payments found for this user');
            }
            ...
            console.log('Opening invoice in browser with data:', invoiceData); // Debug log
            const result = await openInvoiceInBrowser(invoiceData);
            console.log('Invoice opened in browser successfully'); // Debug log
            return result;
        } catch (err) {
            console.error('Open invoice in browser error:', err);
            return { success: false, error: err.message };
        }
    });
